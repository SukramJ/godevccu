...
// WriteResponse frames a single-value BIN-RPC response to w. A nil value
// is encoded as an empty string, matching CUxD convention.
...
// WriteFault frames a BIN-RPC fault packet.
func WriteFault(w io.Writer, fault *xmlrpc.Fault) error {
	if fault == nil {
		return errors.New("binrpc: WriteFault: nil fault")
	}
	if fault.Code < math.MinInt32 || fault.Code > math.MaxInt32 {
		return fmt.Errorf("binrpc: fault code %d out of int32 range", fault.Code)
	}
	payloadStruct := xmlrpc.StructValue{Members: []xmlrpc.Member{
		{Name: "faultCode", Value: xmlrpc.IntValue(int32(fault.Code))},
...
// writeFrame emits the 8-byte header plus payload.
...
// writeParamArray writes `<count:u32><value>…`.
func writeParamArray(w io.Writer, params []xmlrpc.Value) error {
	if err := binary.Write(w, binary.BigEndian, uint32(len(params))); err != nil {
...
// writeValue emits `<typeTag:u32><payload>`.
func writeValue(w io.Writer, v xmlrpc.Value) error {
	switch x := v.(type) {
	case xmlrpc.NilValue:
		// BIN-RPC has no nil tag; represent it as an empty string.
...
// writeRawString writes `<length:u32><ISO-8859-1 bytes>` with no type
// tag — the shape used for a method name and for a typeString tail.
func writeRawString(w io.Writer, s string) error {
	raw, err := latin1Encode(s)
	if err != nil {
		return err
	}
	if lenErr := binary.Write(w, binary.BigEndian, uint32(len(raw))); lenErr != nil {
		return lenErr
	}
	_, err = w.Write(raw)
	return err
...
func writeArray(w io.Writer, a xmlrpc.ArrayValue) error {
	...
	if err := binary.Write(w, binary.BigEndian, uint32(len(a))); err != nil {
		return err
	}
	for i, v := range a {
		if v == nil {
...
// Inf are rejected — the format cannot represent them.
