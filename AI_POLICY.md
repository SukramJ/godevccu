...
gates: `go test -race ./...` (`make test`), `gofmt`, `go vet ./...`, and `golangci-lint run`
(`make lint`) — the same checks CI runs on every pull request. AI assistance does not lower
the bar.

## How this project itself uses AI

In the interest of transparency: `godevccu` is partly developed with AI assistance
